...
function TOOL:StartRightClick()
	...
	elseif self:GetCachedData("GetHoveredObjectOnlySelected") then
		...
		if self:EActive() then
			local many = table.Count(self:GetSelectedObjects())
			if many > 1 then
				local objects = table.GetKeys(self:GetSelectedObjects())
				objects = util.TableToJSON(objects)
				objects = util.Compress(objects)
				local size = #objects
				net.Start("Trolleybus_System.ContactNetworkEditor")
					net.WriteUInt(8,4)
					net.WriteUInt(size,32)
					net.WriteData(objects,size)
				net.SendToServer()
			else
				net.Start("Trolleybus_System.ContactNetworkEditor")
					net.WriteUInt(3,4)
...
function TOOL.BuildCPanel(pnl)
	...
	properties.SetupProperties = function(self,objs)
		...
		for type,objs in pairs(types) do
			...
			if (select(-1,next(obj.Cfg.Properties)).Order)!=nil then
				for k,v in SortedPairsByMemberValue(obj.Cfg.Properties,"Order",false) do
