...
function ENT:Initialize()
	...
	self.BortNumber = 0
	self.DeltaSpawnTime = self.IsTrailer and 0 or math.Rand(1,3)
...
function ENT:ShouldRenderClientEnts(transmit)
	if transmit then
		if self.UpdateTime-self.SpawnTime < 5 + self.DeltaSpawnTime then
			return false
		end

		local plys = self.IsTrailer and self:GetMainTrolleybus():GetTrolleybusOwner()==LocalPlayer() or self:GetTrolleybusOwner()==LocalPlayer()
		if Trolleybus_System.GetPlayerSetting("RenderTrolleybusOptions")==1 then
			return plys
		end
		return self:GetPos():DistToSqr(Trolleybus_System.EyePos())<Trolleybus_System.GetPlayerSetting("TrolleybusDetailsDrawDistance")^2 and !Trolleybus_System.RunEvent("Trolleybus_PreventRenderClientEnts",self)
	end
	return false
end
...
function ENT:DrawTranslucent(flags)
	...
	if self.UpdateTime-self.SpawnTime <= 5 + self.DeltaSpawnTime then
		local bm,bx = self:GetModelBounds()
		local delta = self.UpdateTime-self.SpawnTime
		local eang = Trolleybus_System.EyeAngles()
		cam.Start3D2D(self:GetPos()+self:GetAngles():Forward()*bx.x+self:GetAngles():Right()*bx.y+self:GetAngles():Up()*bx.z,Angle(0,eang.yaw-90,90),1)
			draw.DrawText("Render in: "..(math.floor(5+self.DeltaSpawnTime-delta)).." s","Default",0,0,color_white,TEXT_ALIGN_RIGHT)
		cam.End3D2D()
	end
